...
static uint8_t wait_ready(void){
	uint8_t res;
	uint16_t cnt=0xffff;
	spi_sd_transfer(SDSPI, 0xff);
	do res = spi_sd_transfer(SDSPI, 0xff); while ((res!=0xFF)&& --cnt );
	return res;
	}

static uint8_t send_cmd (uint8_t cmd,uint32_t arg){
	my_printf("sending command : %d\r\n", cmd);
	uint8_t n, res;
	//SDCS_DOWN();
	...
	/* Send command packet */
	spi_sd_transfer(SDSPI, cmd);					/* Start + Command index */
	spi_sd_transfer(SDSPI,(uint8_t)(arg >> 24));	/* Argument[31..24] */
	spi_sd_transfer(SDSPI,(uint8_t)(arg >> 16));	/* Argument[23..16] */
	spi_sd_transfer(SDSPI,(uint8_t)(arg >> 8));	/* Argument[15..8] */
	spi_sd_transfer(SDSPI,(uint8_t)arg);			/* Argument[7..0] */
	n = 0x01;								/* Dummy CRC + Stop */
	if (cmd == CMD0) n = 0x95;				/* Valid CRC for CMD0(0) */
	if (cmd == CMD8) n = 0x87;				/* Valid CRC for CMD8(0x1AA) */
	spi_sd_transfer(SDSPI,n);
	/* Receive command response */
	if (cmd == CMD12) spi_sd_transfer(SDSPI,0xff);	
	/* Skip a stuff byte when stop reading */
    /* Wait for a valid response in timeout of 10 attempts */
	n = 10;
	do res=spi_sd_transfer(SDSPI,0xff); while ((res & 0x80) && --n);
...
uint8_t sd_read_block(uint8_t *buf, uint32_t lba){
	...
	spi_sd_transfer(SDSPI,0xff);
	cnt=0;
	do result=spi_sd_transfer(SDSPI,0xff); while ((result!=0xFE)&&--cnt);
	if(!cnt){SDCS_UP(); return 5;}
	
	for (cnt=0;cnt<512;cnt++) *buf++=spi_sd_transfer(SDSPI,0xff); 
	//получаем байты блока из шины в буфер
	spi_sd_transfer(SDSPI,0xff); //Пропускаем контрольную сумму
	spi_sd_transfer(SDSPI,0xff);
	SDCS_UP();
	spi_sd_transfer(SDSPI,0xff);
	return 0;
...
uint8_t sd_write_block (uint8_t *buf, uint32_t lba){
	...
	spi_sd_transfer(SDSPI,0xff);
	spi_sd_transfer(SDSPI,0xfe);//Начало буфера
	for (cnt=0;cnt<512;cnt++) spi_sd_transfer(SDSPI,buf[cnt]); //Данные
	spi_sd_transfer(SDSPI,0xff);
	spi_sd_transfer(SDSPI,0xff);
	result=spi_sd_transfer(SDSPI,0xff);
	//result=wait_ready();
...
uint8_t sd_init(void){
	...
	uint16_t i;
	for (n = 10; n; n--) spi_sd_transfer(SDSPI,0xff);	/* 80 dummy clocks */
	ty = 0;
	...
	if (send_cmd(CMD8, 0x1AA) == 1){
       /* Get trailing return value of R7 response */
		for (n = 0; n < 4; n++) ocr[n] = spi_sd_transfer(SDSPI,0xff);
